...
app = FastAPI(title='Nest voice-chatterbox', version='0.1.2')
...
def spell_identifier(value: str):
    spoken = []
    for char in value:
        if char == '-':
            spoken.append('dash')
            ...
            spoken.append(char.upper())
        else:
            spoken.append(DIGIT_NAMES.get(char, char))
    return ' '.join(spoken)


def spell_task_id(value: str):
    # Say task ids as "task T nine two..." rather than letting the
    # model blend "t_<hex>" into an unintelligible word.
    body = value.split('_', 1)[1] if '_' in value else value
    return 'task T ' + spell_identifier(body)


def spell_agent_request_id(value: str):
    body = value[3:] if value.lower().startswith('ar-') else value
    return 'agent request A R dash ' + spell_identifier(body)
...
def normalize_url(scheme: str, value: str):
    ...
    while value and value[-1] in '.!?':
        ...
        value = value[:-1]
    parts = value.split('/', 1)
    host = parts[0].replace('-', ' dash ').replace('.', ' dot ')
    path = parts[1].replace('-', ' dash ').replace('_', ' underscore ').replace('/', ' slash ') if len(parts) > 1 else ''
    return scheme + ' link ' + host + ((' slash ' + path) if path else '') + suffix


def normalize_notification_phrases(text: str):
    # Chatterbox was breathing/gasping at a hard phrase boundary after
    # "Agent request received" in Telegram notification copy.  Keep
    # the visible text canonical and smooth only the speech input.
    return re.sub(
        r'\bAgent request received\b[\s:;.!?—–-]*',
...
def normalize_for_tts(text: str):
    normalized = normalize_notification_phrases(text)
    ...
    normalized = re.sub(r'\b(ar-\d{8}-\d{6}-[0-9a-f]+)\b', lambda m: spell_agent_request_id(m.group(1)), normalized, flags=re.IGNORECASE)
    normalized = re.sub(r'\b(?:task\s+)?(t_[0-9a-f]+)\b', lambda m: spell_task_id(m.group(1)), normalized, flags=re.IGNORECASE)
...
def health():
    ...
    return {
        ...
        'supported_generation_options': {
            ...
            'text_normalization': 'smooth Agent request received phrase boundary, spell task/request ids and operational tokens',
        },
...
def speech(request: SpeechRequest):
    ...
    headers = {
        ...
        'X-Nest-TTS-Normalized-SHA256': hashlib.sha256(input_text.encode('utf-8')).hexdigest(),
