function nest::kubernetes::bucket_config(String $name, String $namespace = $nest::kubernetes::namespace) >> Optional[Hash] {
  $kubectl_get_configmap = "kubectl get configmap -n ${namespace} ${name}-bucket -o json 2>/dev/null || echo '{}'"
  $configmap = generate('/bin/sh', '-c', $kubectl_get_configmap).parsejson

  $kubectl_get_secret = "kubectl get secret -n ${namespace} ${name}-bucket -o json 2>/dev/null || echo '{}'"
