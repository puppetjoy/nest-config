class nest::app::hermes::install {
  ...
  $patch_execs = [
    ...
    'patch_hermes_agent_directory_register_refreshes_last_seen',
    ...
    'patch_hermes_kanban_agent_request_failure_notification_posttxn',
  ...
  exec { 'cleanup_hermes_patch_artifacts':
    ...
    require => Exec[$patch_execs + $broker_patch_execs],
  }
  ...
  exec { 'install_hermes_agent':
    ...
    require     => [
      ...
      Exec['patch_hermes_agent_directory_register_refreshes_last_seen'],
  ...
  file { "${install_dir}/agent-directory-register-refreshes-last-seen.patch":
    ensure => file,
    source => 'puppet:///modules/nest/app/hermes/agent-directory-register-refreshes-last-seen.patch',
    mode   => '0644',
    owner  => 'root',
    group  => 'root',
  }
  ...
  exec { 'patch_hermes_agent_directory_register_refreshes_last_seen':
    command => "/usr/bin/patch -N -p1 -d ${source_dir} < ${install_dir}/agent-directory-register-refreshes-last-seen.patch",
    unless  => "/bin/grep -q 'test_register_update_refreshes_last_seen_for_current_profile' ${source_dir}/tests/tools/test_agent_directory_tools.py && /bin/grep -q '\"last_seen_at\": int(args.get(\"last_seen_at\") or now)' ${source_dir}/tools/agent_directory_tools.py",
    require => [
      File["${install_dir}/agent-directory-register-refreshes-last-seen.patch"],
      Exec['patch_hermes_agent_directory_current_profile_resolution'],
    ],
  ...
  file { "${install_dir}/kanban-prod-smoke-lowlevel-guard.patch":
    ensure => file,
    source => 'puppet:///modules/nest/app/hermes/kanban-prod-smoke-lowlevel-guard.patch',
    mode   => '0644',
    owner  => 'root',
    group  => 'root',
  }
  ...
  exec { 'patch_hermes_kanban_prod_smoke_lowlevel_guard':
    command => "/usr/bin/patch -N -p1 -d ${source_dir} < ${install_dir}/kanban-prod-smoke-lowlevel-guard.patch",
    unless  => "/bin/grep -q '_reject_prod_agent_requests_smoke_task' ${source_dir}/hermes_cli/kanban_db.py && /bin/grep -q 'test_create_task_rejects_prod_board_watchdog_smoke_created_by' ${source_dir}/tests/hermes_cli/test_kanban_db.py",
    require => [
      File["${install_dir}/kanban-prod-smoke-lowlevel-guard.patch"],
      Exec['patch_hermes_kanban_prod_smoke_board_guard'],
    ],
  ...
  file { "${install_dir}/kanban-auto-resume-structured-blockers.patch":
    ensure => file,
    source => 'puppet:///modules/nest/app/hermes/kanban-auto-resume-structured-blockers.patch',
    mode   => '0644',
    owner  => 'root',
    group  => 'root',
  }
  ...
  exec { 'patch_hermes_kanban_auto_resume_structured_blockers':
    command => "/usr/bin/patch -N -p1 -d ${source_dir} < ${install_dir}/kanban-auto-resume-structured-blockers.patch",
    unless  => "/bin/grep -q 'auto_resume_satisfied_blockers' ${source_dir}/hermes_cli/kanban_db.py && /bin/grep -q 'test_auto_resume_satisfied_restart_required_blocker' ${source_dir}/tests/hermes_cli/test_kanban_db.py",
    require => [
      File["${install_dir}/kanban-auto-resume-structured-blockers.patch"],
      Exec['patch_hermes_kanban_prod_smoke_lowlevel_guard'],
    ],
  ...
  exec { 'patch_hermes_kanban_requeue_interrupted_worker_owner_exit':
    command => "/usr/bin/patch -N -p1 -d ${source_dir} < ${install_dir}/kanban-requeue-interrupted-worker-owner-exit.patch",
