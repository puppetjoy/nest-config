...
SCHEMAS: list[tuple[dict[str, Any], Any]] = [
    ...
    ({"name": "secure_browser_query", "description": "Versioned page read. v1 returns unsupported; opt-in v2 supports a bounded generated DOM read grammar, not arbitrary JavaScript or page mutation.", "parameters": {"type": "object", "properties": {"expression": {"type": "string"}}}}, secure_browser_query_tool),
    ...
    ({"name": "secure_browser_click", "description": "Click a fresh accessibility locator or visible coordinate in the canonical Firefox handoff tab, then return bounded transition-aware readback. A delivered click is never confirmed completion. Joy's workflow direction is the authorization boundary; use action_key for exactly-once destructive/financial actions.", "parameters": {"type": "object", "properties": {"locator": {"type": "string"}, "coordinate": {"type": "array", "items": {"type": "integer"}, "minItems": 2, "maxItems": 2}, "workflow_id": {"type": "string"}, "action_key": {"type": "string"}, "idempotency_key": {"type": "string"}, "max_wait_seconds": {"type": "number", "minimum": 0, "maximum": 15}, "selector": {"type": "string", "description": "CSS selector point is supported only in opt-in firefox-bidi-ui-v2; v1 rejects it"}, "approved_effect": {"type": "string", "description": "Legacy audit label; not an approval gate"}}, "anyOf": [{"required": ["locator"]}, {"required": ["coordinate"]}, {"required": ["selector"]}]}}, secure_browser_click_tool),
    ({"name": "secure_browser_type", "description": "Type bounded non-secret text into a fresh accessibility locator and return redacted readback. Never pass passwords, payment numbers, tokens, or other secrets; operate Firefox/Bitwarden UI instead.", "parameters": {"type": "object", "properties": {"locator": {"type": "string"}, "selector": {"type": "string", "description": "CSS selector point is supported only in opt-in firefox-bidi-ui-v2; v1 rejects it"}, "text": {"type": "string", "maxLength": 4096}, "workflow_id": {"type": "string"}}, "required": ["text"]}}, secure_browser_type_tool),
