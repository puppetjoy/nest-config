...
def _load_tool_module():
    # Source checkouts execute this helper beside shopping_browser_tool.py;
    # deployed units run with PYTHONPATH=/opt/hermes-agent/src and import the
    # Puppet-copied tool from tools.shopping_browser_tool.
    here = Path(__file__).resolve()
    local_tool = here.with_name("shopping_browser_tool.py")
    if local_tool.exists():
        spec = importlib.util.spec_from_file_location("shopping_browser_tool", local_tool)
    ...
    for parent in here.parents:
        ...
        if (candidate / "shopping_browser_tool.py").exists():
            sys.path.insert(0, str(parent))
            from tools import shopping_browser_tool  # type: ignore[import-not-found]

            return shopping_browser_tool
    from tools import shopping_browser_tool  # type: ignore[import-not-found]
