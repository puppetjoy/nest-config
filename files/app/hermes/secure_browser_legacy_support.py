"""Temporary retail-order/consumable support from the retired CDP bridge.

Only ``retail_order_*`` and ``consumable_*`` handlers register from this
module. The browser handlers remain temporarily importable for the order
refresh implementation but are not exposed; ``secure_browser_tool.py`` owns
the non-instrumented Firefox UI surface. Remove the dead browser implementation
after order refresh no longer imports its helper functions.
"""
...
class _RetailStateRegistry:
    """Keep the retired bridge implementation importable without re-registering it."""

    @staticmethod
    def register(**kwargs: Any) -> Any:
        name = str(kwargs.get("name") or "")
        if name.startswith(("retail_order_", "consumable_")):
            return _registry.register(**kwargs)
        return None


registry = _RetailStateRegistry()
...
def _compact_owner_checkout_review_result(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "minimal_order_facts": _minimal_owner_checkout_facts(checkout_review),
        "retention": _bounded_checkout_scalar(data.get("retention"), 200),
        "checkout_handoff_default": "Star may submit ordinary final order controls through secure_browser_click approved_effect='final_purchase' when Joy directs; this owner-only review is optional evidence, not a required approval ceremony.",
        "safety_boundary": "Complete checkout screenshots were sent only to Joy via the trusted Telegram path. This acknowledgement omits raw screenshots, file paths, DOM, cookies, storage, request headers, and full address/payment/account text. Owner-only review is evidence only; Joy-directed final purchase remains available through secure_browser_click approved_effect='final_purchase' unless login, credential/security challenge, or payment/address/account edit scope appears; ledger/order confirmation remains post-click evidence.",
    }
...
def _amazon_your_orders_observation(order: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "status": "skipped",
        "reason": "background browser refresh is disabled under firefox-ui-v1; read-only Gmail refresh remains available",
    }
...
def _order_entry_from_final_purchase_result(result: dict[str, Any], checkout_summary: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    proof_value = result.get("post_purchase_proof")
    proof: dict[str, Any] = proof_value if isinstance(proof_value, dict) else {}
    confirmed_proof = _post_purchase_proof_confirms_final_purchase(proof)
    ...
    return _upsert_order_entry({
        ...
        "status": "confirmed" if confirmed_proof else "pending_confirmation",
        "eta_window": eta,
        ...
        "source_refs": ["trusted_final_purchase_executor", "owner_post_purchase_proof" if confirmed_proof else ("post_purchase_proof_not_confirmed" if proof else "post_purchase_proof_not_available")],
        "refresh_sources": ORDER_REFRESH_SOURCE_PRIORITY,
        "notes": "Created automatically by trusted final-purchase executor after a successful final purchase click; status remains pending unless owner-only post-purchase proof includes a thank-you/order-confirmation signal.",
    })
...
def _submit_final_purchase_approval_request(summary: dict[str, Any], material_summary_binding: str, owner_visual_evidence_binding: str, owner_review_id: str, note: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        proposal_text = "\n".join([
            ...
            "This legacy approval path is optional and does not remove Star's Joy-directed secure_browser_click approved_effect='final_purchase' path; neither path authorizes payment/address/account edits, subscriptions, add-ons, warranty/protection changes, login, passkeys, 2FA, CAPTCHA, or security prompts.",
        ])
...
def _request_final_purchase_approval(material_summary_binding: str, owner_visual_evidence_binding: str, owner_review_id: str = "", note: str = "") -> dict[str, Any]:
    ...
    def run(browser: CdpSession) -> dict[str, Any]:
        ...
        _audit("final_purchase_approval_request_retired", {"status": "retired_use_secure_browser_click", "material_summary_binding": expected_binding, "owner_visual_evidence_binding": evidence_binding, "owner_review_id": owner_review_id})
        return {
            "operation": "request_final_purchase_approval",
            "status": "retired_use_secure_browser_click",
            ...
            "replacement_action": "Call secure_browser_click on the visible final order-submission control with approved_effect='final_purchase' when Joy directs Star to place the order.",
            "safety_boundary": "The legacy Talon Agent Request final-purchase approval handoff is retired for Star full-access shopping. This tool no longer files an Agent Request or blocks the checkout lane; post-purchase proof and ledger status are reported after Star's Joy-directed click.",
        }
...
def _execute_final_purchase(approval_request_id: str, material_summary_binding: str, owner_visual_evidence_binding: str) -> dict[str, Any]:
    ...
    def run(browser: CdpSession) -> dict[str, Any]:
        ...
        transition = _wait_for_final_purchase_transition(browser, session_id, target_id, str(summary.get("url") or ""), str(summary.get("page_title") or ""))
        raw_final_url = str(transition.get("url") or "about:blank")
        raw_final_title = str(transition.get("page_title") or "")
        final_url = _sanitize_url(raw_final_url)
        final_title = _sanitize_shopping_text(raw_final_title)
        post_purchase_proof: dict[str, Any] = {}
        transition_session_id = str(transition.get("session_id") or session_id)
        if transition.get("state") == "post_purchase_confirmation_available" and _is_amazon_post_purchase_page(raw_final_url, raw_final_title):
            try:
                post_purchase_proof = _owner_post_purchase_review_from_attached(browser, transition_session_id, raw_final_url, raw_final_title, retain_local=False)
            except Exception as exc:
                post_purchase_proof = {
                    "status": "post_purchase_owner_proof_failed",
                    "message": str(exc)[:500],
                    "post_purchase_readback": {k: v for k, v in transition.items() if k != "session_id"},
            ...
            post_purchase_proof = {
                ...
                "reason": str(transition.get("reason") or "post-purchase confirmation unavailable")[:240],
                "post_purchase_readback": {k: v for k, v in transition.items() if k != "session_id"},
                "verification_next_steps": transition.get("verification_next_steps"),
                "safety_boundary": "Final purchase click was submitted, but a sanitized post-purchase confirmation was unavailable from the browser. Verify via Gmail, Amazon Your Orders, or the safe retail order ledger; no raw order/address/payment/browser evidence is exposed to Star.",
            }
        with _final_purchase_state_lock() as handle:
            state = _load_final_purchase_state(handle)
            tokens = state.setdefault("tokens", {})
        ...
        result = {
            ...
            "status": str(transition.get("status") or "clicked"),
            "request_id": request_id,
            "approval_id": approval_id,
            "material_summary_binding": expected_binding,
            "owner_visual_evidence_binding": evidence_binding,
...
def _checkout_review_page_rank(page: dict[str, Any]) -> tuple[int, str]:
    ...
    # They should also win over stale Amazon Your Orders tabs: owner checkout
    # review is often called while an older order-history tab is still open, and
    # that generic surface must not be mistaken for the current live checkout.
    if _is_checkout_upsell_interstitial_page(url, title):
        return (0, url)
    if _is_checkoutish_page(url, title) and not _is_amazon_post_purchase_page(url, title):
        return (3, url)
    if _is_amazon_post_purchase_page(url, title) and POST_PURCHASE_CONFIRMATION_RE.search(" ".join([url, title])):
        return (2, url)
...
def _sanitize_checkout_summary(summary: dict[str, Any], safety: dict[str, Any], controls: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    blocked_metadata: dict[str, Any] = {
        ...
        "final_purchase_policy": "Joy-directed final Buy Now, Place Order, Pay, or equivalent order-submission controls are available to Star through secure_browser_click approved_effect='final_purchase' on visible HTTPS shopping/cart/checkout/order-review controls. Star must still pause for login, credential/security challenges, payment/address/account edits, and sensitive-information prompts. Post-click proof and ledger confirmation are reporting layers, not click gates.",
    }
    ...
    sanitized: dict[str, Any] = {
        ...
        "checkout_prep_control_policy": "Star may inspect and use sanitized selectors/labels for ordinary checkout-prep controls with an explicit approved_effect, including Joy-directed final purchase via approved_effect='final_purchase'. Address/payment/account/security edits and credential challenges remain Joy-only; post-click evidence and retail ledger confirmation are reporting layers rather than blockers.",
        "blocked_metadata": blocked_metadata,
        "policy": "Sanitized checkout-prep/order-review summary only: structured fields are isolated; street addresses, full payment/account/card numbers, emails, phone numbers, security-code text, raw DOM, cookies, and storage are not returned. Final purchase controls are exposed only as sanitized click targets when Star uses approved_effect='final_purchase' at Joy's direction.",
    }
    binding_material = {
...
def _unconfirmed_post_purchase_tracking_result(reason: str) -> dict[str, Any]:
    return {
        "status": "ledger_entry_not_created",
        "reason": reason,
        "safety_boundary": "Generic order-history visibility is not proof of a new order. No confirmed retail-order ledger row was created without a thank-you/order-confirmation signal from the just-completed checkout.",
    }


def _post_purchase_proof_confirms_final_purchase(proof: dict[str, Any]) -> bool:
    if not isinstance(proof, dict) or not proof:
        return False
    proof_status = str(proof.get("status") or "")
    if proof_status in {
        "ambiguous_post_purchase_evidence",
        "submitted_confirmation_unavailable",
        "post_purchase_owner_proof_failed",
        "not_attempted",
    }:
        return False
    candidates = []
    for key in ("post_purchase_review", "post_purchase", "minimal_post_purchase_facts"):
        value = proof.get(key)
        if isinstance(value, dict):
            candidates.append(value)
    return any(_post_purchase_summary_confirms_final_purchase(candidate, str(proof.get("url") or ""), str(proof.get("page_title") or "")) for candidate in candidates)
...
def _assert_checkout_click_allowed(metadata: dict[str, Any], effect: str, reason: str) -> None:
    ...
    if FINAL_PURCHASE_RE.search(control_identity_text):
        if effect != "final_purchase":
            raise ValueError("final purchase controls require approved_effect=final_purchase so post-click readback uses the final-purchase transition path")
        return
...
def _assert_checkout_type_allowed(metadata: dict[str, Any], effect: str, reason: str, typed_text: str) -> None:
    ...
    if FINAL_PURCHASE_RE.search(_checkout_control_identity_text(metadata)):
        raise ValueError("final purchase controls cannot be modified by secure_browser_type; use secure_browser_click with approved_effect='final_purchase' at Joy's direction")
    _assert_checkout_control_not_sensitive(control_text)
...
def _reject_unsafe_operation(operation: str) -> dict[str, Any]:
    ...
    if op in ("browse", "navigate", "page_snapshot", "query", "order_history", "buy_again", "past_order_details"):
        return {
            ...
            "message": "Allowed for ordinary shopping, account research, including Amazon order history and Buy Again pages. Outputs are sanitized; Star must pause for login, Bitwarden, passkeys, 2FA/CAPTCHA, suspicious security prompts, and payment/address/account edits. Joy-directed final purchase uses secure_browser_click approved_effect='final_purchase'.",
        }
    if op == "checkout":
        return {
            "allowed": True,
    ...
    if op in ("request_final_purchase_approval", "final_purchase_approval"):
        return {
            ...
            "boundary": "legacy_talon_handoff_retired",
            "message": "Legacy Agent Request final-purchase approval handoffs are retired for Star full secure-browser access. Star should use secure_browser_click with approved_effect='final_purchase' when Joy directs the order submission, then record sanitized post-purchase/ledger evidence after the fact.",
        }
    ...
    if op in UNSAFE_OPERATIONS:
        return {
            ...
            "operation": op,
            "message": "Star's shopping bridge allows scoped browsing, visible-page inspection, safe queries, careful clicks/types, cart inspection, explicitly approved add-to-cart, checkout prep, owner-directed final-purchase clicks, and explicitly approved visible cart line-item removal. Joy handles login, Bitwarden, passkeys, 2FA, CAPTCHA, account, address, and payment actions manually.",
        }
...
def _owner_checkout_review_page_info(browser: CdpSession) -> dict[str, str]:
    ...
    if (
        _is_owner_checkout_review_page(page_url, page_title)
        and not _is_checkout_upsell_interstitial_page(page_url, page_title)
        and not _is_amazon_post_purchase_page(page_url, page_title)
    ):
        return page_info
    candidates = [
        page
...
def _owner_checkout_review_page_error(url: str, title: str) -> str:
    ...
    return (
        ...
        "Secure checkout control, then retry owner review. Joy-directed final purchase "
        "uses secure_browser_click approved_effect='final_purchase' on the order-review page."
    )
...
def _owner_post_purchase_review_from_attached(browser: CdpSession, session_id: str, url: str, title: str, retain_local: bool = False) -> dict[str, Any]:
    ...
    confirmed_new_order = _post_purchase_summary_confirms_final_purchase(post_purchase, url, title)
    ...
    if confirmed_new_order:
        ...
        result["status"] = "ambiguous_post_purchase_evidence"
        result["shopping_order_tracking"] = _unconfirmed_post_purchase_tracking_result(
            "Amazon Your Orders/order-history was visible without a final-purchase thank-you or order-confirmation signal"
        )
...
def _owner_checkout_review_caption(review_id: str, index: int, count: int, binding: str, mode: str, checkout_review: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Star received only the redacted acknowledgement/summary; Joy-directed final purchase remains available through secure_browser_click approved_effect='final_purchase'."
        f"{total_note}"
    )
    ...
    def run(browser: CdpSession) -> dict[str, Any]:
        ...
        if not binding:
            ...
            raise RuntimeError(
                ...
                "review on the order-review page. Joy-directed final purchase "
                "uses secure_browser_click approved_effect='final_purchase'."
            )
...
def _click(selector: str, reason: str, approved_effect: str) -> dict[str, Any]:
    ...
    def run(browser: CdpSession) -> dict[str, Any]:
        ...
        selectorless_final_purchase = False
        if effect == "remove_from_cart":
            ...
            if effect == "final_purchase" and not checkout_metadata.get("exists"):
                safety = _evaluate(browser, session_id, CHECKOUT_PAGE_SAFETY_JS) or {}
                if safety.get("blocked_reason"):
                    raise ValueError(str(safety.get("blocked_reason")))
                page_metadata = {
                    "url": safety.get("url") or source_url,
                    "page_title": safety.get("page_title"),
                }
        ...
        if selectorless_final_purchase:
        ...
        if clicked_final_purchase:
            transition = _wait_for_final_purchase_transition(browser, session_id, resolved_target_id, source_url, str(_evaluate(browser, session_id, "document.title") or current_title or ""))
        ...
        if clicked_final_purchase:
            ...
            result["final_purchase_submission"] = {
                ...
                "status": "final_purchase_not_clicked",
                "reason": _sanitize_shopping_text(str(result.get("reason") or "final purchase control was not clicked"))[:240],
                "source_url": _sanitize_url(source_url),
                "source_page_title": _sanitize_shopping_text(str(current_title or ""))[:240],
                "safety_boundary": "No final-purchase success or retail-order ledger entry is reported unless the final control click succeeds and the post-click readback proves or explicitly bounds the purchase state.",
            }
        elif effect in CHECKOUT_APPROVED_EFFECTS:
...
CLICK_SCHEMA = {
    ...
    "description": "Click a visible element in the persistent secure browser by CSS selector. Use for browsing, selecting variants/options, applying visible coupons, explicitly approved add-to-cart/removal, checkout-prep controls, and Joy-directed final purchase controls. Checkout/order-review clicks require an explicit approved_effect such as checkout_prep, select_shipping_option, select_delivery_option, select_packaging_option, apply_checkout_option, fix_purchase_mode, cart_line_adjustment, or final_purchase and return refreshed sanitized readback where available. Never use for account/payment/address edits, login, passkeys, 2FA, CAPTCHA, or credential/security challenges.",
    "parameters": {
        "type": "object",
        "properties": {
...
OWNER_CHECKOUT_REVIEW_SCHEMA = {
    ...
    "description": "Send complete sensitive checkout/order-review or post-purchase confirmation/order-verification visual evidence directly to Joy's configured Telegram destination for owner-only review. It first attempts full-document capture; if that is unavailable, it sends an ordered viewport sequence covering the page from top through below-the-fold material sections. For pre-purchase checkout, intended for verification of address, payment, item, delivery, tax/total, discounts, and final controls. For post-purchase pages, intended for thank-you/confirmation and Your Orders delivery proof. Returns only a redacted acknowledgement, bindings, and sanitized summary to Star; it never returns image paths, MEDIA handles, raw DOM, cookies, storage, request headers, CDP endpoints, credentials, passkeys, 2FA/CAPTCHA data, raw order numbers, or structured address/payment details. Owner-only review is optional evidence; Joy-directed final purchase uses secure_browser_click approved_effect='final_purchase' and ledger confirmation remains post-click reporting.",
    "parameters": {
        "type": "object",
        "properties": {
...
if __name__ == "__main__":
    ...
    assert "approved_effect='final_purchase'" in mixed_checkout_summary["blocked_metadata"]["final_purchase_policy"]
    ...
    assert non_amazon_approval["status"] == "retired_use_secure_browser_click"
    assert "secure_browser_click" in non_amazon_approval["replacement_action"]
    try:
    ...
    assert _reject_unsafe_operation("request_final_purchase_approval")["allowed"] is False
    assert "retired" in _reject_unsafe_operation("request_final_purchase_approval")["message"]
