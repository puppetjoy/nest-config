# frozen_string_literal: true

PuppetLint.new_check(:deploy_plan_boundary) do
  DEPLOY_PLAN = %r{^\s*plan\s+\S*deploy\S*\s*\(}

  def check
    manifest_lines.each_with_index do |line, index|
      next unless line.match?(DEPLOY_PLAN)

      notify(
        :error,
        {
          message: 'plans with deploy in the name must be YAML deployment wrappers; use puppet-lint native control comments only for deliberate imperative exceptions',
          line: index + 1,
          column: line.index('plan') + 1,
        },
      )
    end
  end
end
