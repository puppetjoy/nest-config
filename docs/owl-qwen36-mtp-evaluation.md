...
Q8-family benchmark proved the plain Q8_0 MTP artifact can run with `--mmproj`
and the same per-slot 262144 context, so the canonical service can use MTP as
long as rollout verification preserves the four-slot/vision topology.
...
- `model_file`: `Qwen3.6-35B-A3B-MTP-Q8_0.gguf`
- `model_path`: `/cache/models/Qwen3.6-35B-A3B-MTP-Q8_0.gguf`
- `model_url`: `https://huggingface.co/unsloth/Qwen3.6-35B-A3B-MTP-GGUF/resolve/main/Qwen3.6-35B-A3B-Q8_0.gguf`
...
The MTP repository publishes the Q8_0 artifact without an MTP prefix, so the
source-managed cache path names it explicitly as `MTP-Q8_0` while downloading
from the repository's `Qwen3.6-35B-A3B-Q8_0.gguf` file.
...
Plain Q8_0 MTP is the rollout candidate because it was faster in absolute token
generation than UD-Q8_K_XL MTP while using less GTT.  The measured speedup over
non-MTP Q8_0 was +34.66% token generation for about +1.89 GiB GTT.
